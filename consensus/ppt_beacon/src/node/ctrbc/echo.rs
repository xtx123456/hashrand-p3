...
use types::{beacon::{CoinMsg, CTRBCMsg}, Replica};

use crate::node::{Context, CTRBCState};

impl Context {
    #[async_recursion]
    pub async fn process_echo(&mut self, ctrbc: CTRBCMsg, master_root: Hash, echo_sender: Replica) {
        ...
        log::info!(
            "Received ECHO message from {} for secret from {} in round {}",
            echo_sender,
            ctrbc.origin,
            ctrbc.round
        );
        if rbc_state.terminated_secrets.contains(&sec_origin) {
            log::info!(
                "Terminated secretsharing of instance {} already, skipping this echo",
                sec_origin
            );
            return;
        }
        ...
        if mp.root() != master_root || !ctrbc.verify_mr_proof(&self.hash_context) {
            log::error!(
                "Merkle root of WSS Init from {} did not match Merkle root of ECHO from {}",
                sec_origin,
                self.myid
            );
            return;
        ...
        rbc_state.add_echo(sec_origin, echo_sender, &ctrbc);
        let hash_root = rbc_state.echo_check(
            sec_origin,
            self.num_nodes,
            self.num_faults,
            self.batch_size,
        ...
        match hash_root {
            ...
            Some(vec_hash_root) => {
                ...
                self.broadcast(
                    CoinMsg::CTRBCReady(ctrbc.clone(), vec_hash_root.0, self.myid),
                    ctrbc.round,
                )
                .await;
        ...
        self.add_benchmark(
            String::from("process_batch_wssecho"),
