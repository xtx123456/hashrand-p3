...
impl Context {
    #[async_recursion]
    pub async fn process_ready(&mut self, ctrbc: CTRBCMsg, master_root: Hash, ready_sender: Replica) {
        ...
        log::info!(
            "Received READY message from {} for secret from {} in round {}",
            ready_sender,
            ctrbc.origin,
            ctrbc.round
        );
        ...
        if mp.root() != master_root || !ctrbc.verify_mr_proof(&self.hash_context) {
            log::error!(
                "Merkle root of WSS Init from {} did not match Merkle root of READY from {}",
                sec_origin,
                self.myid
            );
            return;
        ...
        match res.1 {
            ...
            Some(root_vec) => {
                if res.0 == self.num_faults + 1 {
                    ...
                    self.broadcast(
                        CoinMsg::CTRBCReady(ctrbc.clone(), root_vec.0, self.myid),
                        ctrbc.round,
                    )
                    .await;
                    ...
                    let ctrbc = CTRBCMsg::new(shard.0.clone(), shard.1.clone(), ctrbc.round, sec_origin);
                    self.broadcast(
                        CoinMsg::CTRBCReconstruct(ctrbc.clone(), master_root.clone(), self.myid),
                        ctrbc.round,
                    )
                    .await;
